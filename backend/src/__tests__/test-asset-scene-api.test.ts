...
import { updateAssetSceneMapping, getAssetsByScene } from '../services/asset.service';
...
describe('Asset-Scene Mapping Service Tests', () => {
  ...
  beforeEach(async () => {
    // Insert test asset
    await db.prepare('INSERT INTO assets (id, name, type, brand, model, serial_number, scene_id, yaw, pitch, floor, room, status, walkthrough_id, org_id, property_id, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)').run(
      testAsset.id,
      testAsset.name,
      testAsset.type,
      testAsset.brand,
      ...
      testAsset.room,
      testAsset.status,
      testAsset.walkthrough_id,
      testAsset.org_id,
      testAsset.property_id,
  ...
  afterEach(async () => {
    // Clean up test asset
    await db.prepare('DELETE FROM assets WHERE id = ?').run(testAsset.id);
  });
  ...
  it('should update asset scene mapping successfully', async () => {
    const result = await updateAssetSceneMapping(testAsset.id, {
  ...
  it('should retrieve assets by scene_id', async () => {
    ...
    await updateAssetSceneMapping(testAsset.id, {
      scene_id: 'target_scene',
      yaw: 90,
      pitch: 0
    });

    const assets = await getAssetsByScene('target_scene');
  ...
  it('should return empty array for scene with no assets', async () => {
    const assets = await getAssetsByScene('empty_scene');
