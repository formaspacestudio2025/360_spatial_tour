	...
	module.exports = function $$$reconciler($$$hostConfig) {
		...
		function updateSuspenseComponent(current, workInProgress, renderLanes) {
			...
			if (current === null) {
				...
				if (prevState !== null) {
					...
					if (_dehydrated !== null) if (!didSuspend) return updateDehydratedSuspenseComponent(current, workInProgress, _dehydrated, prevState, renderLanes);
		...
		*/
		...
		function commitDetachRef(current) {
			...
			if (currentRef !== null) if (typeof currentRef === "function") if (current.mode & ProfileMode) try {
				startLayoutEffectTimer();
				currentRef(null);
			} finally {
				recordLayoutEffectDuration(current);
		...
		function unmountHostComponents(finishedRoot, current, nearestMountedAncestor) {
			...
			while (true) {
				...
				} else if (node.tag === DehydratedFragment) if (currentParentIsContainer) clearSuspenseBoundaryFromContainer(currentParent, node.stateNode);
	...
	function switchInstance(instance, type, newProps, fiber) {
		...
		[fiber, fiber.alternate].forEach((fiber) => {
			if (fiber !== null) {
				...
				if (fiber.ref) if (typeof fiber.ref === "function") fiber.ref(newInstance);
...
function applyProps$1(instance, data) {
	...
	for (let i = 0; i < changes.length; i++) {
		...
		if (value === DEFAULT + "remove") if (currentInstance.constructor) {
		...
		if (isEvent && localState) {
			...
			if (Array.isArray(value)) if (targetProp.fromArray) targetProp.fromArray(value);
...
//# sourceMappingURL=react-three-fiber.esm-CiZt6ojx.js.map